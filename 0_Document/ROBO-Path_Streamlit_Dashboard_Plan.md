...
1. 맥미니(로컬) 터미널에서 아래 명령어를 실행하여 Streamlit 서버를 구동합니다.
   ```bash
...
## 5. 향후 구현 과제 (Future Work): 로그인 및 데이터 소유권 연동

**⚠️ 중요: 본 항목의 실제 구현은 Unity 시뮬레이터와 메인 로직이 완전히 완성된 이후, 웹 공개 준비 단계에서 진행됩니다.**

향후 일반 공개 시 다중 사용자 관리를 위해 Streamlit 대시보드에 사용자 인증 및 소유권 기반 데이터 렌더링 기능을 추가합니다.

*   **로그인 / 회원가입 화면 (UI):**
    *   Streamlit 메인 진입 시 인증 세션을 검사하여, 미인증 사용자는 로그인/가입 화면으로 유도합니다.
    *   가입 폼에는 보안을 위한 '인증키(Secret Key)' 입력란이 포함됩니다.
*   **본인 미션만 조회 흐름:**
    *   `mission_logs`, `incidents` 등의 개인 데이터 테이블 조회 시, 백엔드에서 사용자 세션의 `user_id`를 기반으로 필터링(혹은 Supabase RLS에 의해 자동 필터링)된 결과를 반환받아 화면에 렌더링합니다.
    *   공용 지도 데이터(`nodes`, `map_edges`)는 인증된 모든 사용자에게 동일하게 표출되어 집단 지성 데이터를 렌더링합니다.
