...
- **경사 한계:** 15도 초과 경사 및 계단 통행 불가 (NavMesh는 연결되나 A* 비용 산출 단계에서 차단됨)
...
- **복셀 매핑 시 태그 기록:** 로봇이 레이캐스트로 복셀(노드)을 활성화할 때, 닿은 오브젝트의 태그(`Path_Stair`, `Path_Ramp` 등)를 복셀 데이터에 함께 기록합니다. 이를 통해 복셀 형상이 경사면으로 동일해도 데이터 상으로는 계단과 경사를 구분할 수 있습니다. (향후 Step 3 복셀 감지 구현 시 반영)
- **웹 인터페이스 시각화 규칙:** 매핑된 복셀을 웹 3D 대시보드에서 렌더링할 때, 일반 경사(`Path_Ramp`)는 기본 경사 색상을 적용하고, 계단 경사(`Path_Stair`)는 다른 색상과 함께 **가로 줄무늬(stripe) 패턴**을 적용합니다. 색약/유사 색조 조건에서도 형태(패턴)를 통해 계단임을 명확히 구분하기 위함입니다. (향후 Step 6 웹 시각화 구현 시 반영)

### 3.2 엣지 물리 피드백 연산
로봇이 두 노드 사이의 엣지를 횡단 완료할 때마다, 아래 3대 지표를 산출하여 DB의 플랫폼 통계(`platform_stats` JSONB 필드)에 누적 평균치로 갱신(UPDATE)합니다.

> **[설계 단순화 반영 (Phase 4)]**  
> 구현 시간 및 난이도를 고려하여, 현재 시뮬레이터 연동 단계에서는 '개별 엣지 횡단마다' 산출하는 대신 **'하나의 목적지(SetDestination 단위) 이동 완료 시 1회'** 피드백을 산출하는 것으로 단순화합니다. (세분화된 개별 엣지 피드백은 후속 과제로 남깁니다.)
...
### 6.1 세션 관리 메커니즘 (스냅샷 아카이빙)
- **작업 테이블 기록 원칙:** 시뮬레이터는 항상 현재 Supabase에 활성화된 기본 작업 테이블(`discovered_nodes`, `map_edges` 등)에 데이터를 기록합니다. 시뮬레이터 자체는 복잡한 `session_id`를 관리하거나 삽입하지 않습니다.
- **백엔드 위임:** 세션 전환 시 이전 세션의 데이터를 스냅샷으로 저장하고 새 세션 데이터를 적재하는 등의 세션 격리 처리는 전적으로 파이썬 백엔드(라즈베리파이 스토리지 서버)가 담당합니다. 기존 A* 알고리즘 등 백엔드 로직 역시 수정 없이 작업 테이블만 바라보면 됩니다.
...
### 9.1 로봇 회수(Recall) 시스템

**목적**
- 버그로 로봇이 끼이거나, 좁은 공간에 갇혀 빙빙 도는 등 정상 이동이 불가능한 상황에서, 시뮬레이션 전체를 초기화하지 않고 해당 로봇만 복구하기 위한 운영 기능.
- 회수는 근본적으로 "버그 대응" 목적이 우선이다.

**동작 (필수 사양)**
- 사용자가 관제 UI(Streamlit)에서 특정 로봇을 선택해 '회수' 명령을 내린다.
- 해당 로봇은 즉시 거점(Node_Station)으로 복귀한다. (이동 과정 없이 순간 복귀 또는 즉시 텔레포트)
- 수행 중이던 임무는 **실패(failed)로 처리**된다.
- 회수된 임무 동안 수집된 주행 데이터(L/S/E 등)는 **채택하지 않고 폐기**한다. (비정상 종료된 주행은 신뢰할 수 없으므로. 부분 구간 보존 로직은 두지 않아 구현을 단순화한다.)
- 회수 후 로봇은 거점에서 대기 상태가 되어 다음 임무를 받을 수 있다. (시뮬레이션 전체에는 영향 없음)

**데이터 처리**
- 실패한 임무는 `mission_logs`에 실패 상태로 기록된다. (회수 시 임무를 실패로 기록하려면 `mission_logs`에 status 필드가 필요하나 현재 스키마에는 없다. 구현 시 status 필드를 추가한다(DB 아키텍처 문서 참조).)
- 실패 기록은 통계에서 "실패한 임무"로 집계 가능하며, 특정 구간에서 회수가 빈번하면 해당 지형의 문제 신호로 해석할 수 있다(분석 가치).

**후순위 항목 (여유 시 구현, 현재 범위 아님)**
- 회수 사유 기록(끼임/빙빙돔/기타 선택): 구현 난이도와 마감을 고려하여 우선순위를 낮춘다.
- 회수 전 정상 구간의 부분 데이터 보존: 끼임 지점 판단 로직이 필요해 복잡하므로 후순위.

**구현 시점**
- 로봇 및 미션 시스템이 구축된 이후(Step 3 이후)에 구현한다. 본 문서는 설계 기록이며 현재는 구현하지 않는다.
