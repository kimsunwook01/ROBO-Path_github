...
### 2.1 파일: `Assets/Scripts/MapEditor/Editor/MapEditorWindow.cs`
...
- **배치 모드 시각화 및 좌표 체계:**
  - 격자 중심(+5) 및 블록 밑면(중심 y = baseY + 높이/2) 기준으로 좌표를 결정한다.
  - `Handles.DrawWireCube` 등을 사용하여 현재 마우스가 가리키는 위치에 선택된 블록의 크기와 회전 상태를 미리보기로 렌더링한다. ESC 키로 배치 블록 선택을 해제할 수 있다.
- **입력 이벤트 처리 (`Event.current`):**
  - **면 법선(Normal) 기반 배치:** 레이캐스트에 맞은 면의 방향에 따라 배치 위치가 결정된다 (윗면=위로 적층, 옆면=옆 칸 동일 높이, 아랫면=아래). 블록의 자세(기울기)는 변하지 않는다.
  - **R 키 누름 (KeyDown):** 선택 블록 또는 미리보기 블록의 90도 회전 적용 (Unity 기본 스케일 단축키 R은 맵 에디터 창에서 차단됨).
  - **좌클릭 (MouseDown, button 0):** 계산된 타겟 칸과 높이에 블록을 배치한다.
- **선택 모드:**
  - 좌클릭 시 블록을 선택하며, `Delete` 또는 `Backspace` 키로 선택한 블록을 삭제한다.
  - `W` 키를 통해 Unity 기본 이동 툴을 활성화하여 이동할 수 있다. 우클릭은 Unity 기본 씬 시야 조작으로 사용된다.
