...
* **📈 경험 로그 및 LLM 피드백 (Logs & Feedbacks)**
  * `mission_logs`: 개별 주행 임무 완료 후 도출된 플랫폼별 부하율($L$), 안정성($S$), 효율성($E$) 요약 성적표
    * **참고**: 현재 스키마에 `status` 필드가 없으며, 로봇 회수(Recall) 기능 구현 시 임무 성공/실패 기록용으로 추가가 필요합니다.
...
시뮬레이션 환경의 다중 "세이브 슬롯" 지원을 위해 '세션(Session)' 모델을 도입합니다. 세션 전환은 기존 DB 작업 테이블(Working tables)에 `session_id` 컬럼을 추가하는 방식이 아닌, **"스냅샷 아카이빙(Snapshot Archiving)"** 방식을 사용합니다.
*   **고정 데이터 (세션과 무관하게 영구 유지):** `nodes`의 `base_locations`, `map_metadata`, `robots`, `auth.users`
*   **세션 종속 데이터 (작업 테이블):** `discovered_nodes`, `map_edges`의 `platform_stats`, `mission_logs`, `incidents`, `simulation_logs`
*   **작업 테이블의 정의:** 시뮬레이터와 백엔드 A* 알고리즘은 현재 활성화된 세션에 대해서만 읽고 씁니다. 작업 테이블에는 `session_id` 컬럼이 없으며, 오직 "현재 세션의 데이터"만 존재합니다. 세션을 전환할 때마다 파이썬 백엔드(라즈베리파이)가 작업 테이블 데이터를 통째로 직렬화하여 SSD에 스냅샷으로 저장한 뒤 비우고, 새 세션의 스냅샷을 불러옵니다.
...
CREATE TABLE mission_logs (
    ...
    -- TODO: 로봇 회수(Recall) 기능 구현 시 임무 성공/실패를 기록할 status 필드 추가 필요
