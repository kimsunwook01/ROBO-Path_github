...
namespace ROBOPath.Robot
{
    public class LogTelemetrySink : MonoBehaviour, ITelemetrySink
    {
        public void EmitFeedback(RobotPlatform platform, string fromNodeId, string toNodeId, float? load, float? stability, float? efficiency)
        {
            ...
            {
                Debug.Log($"[Telemetry] Platform: {platform}, Segment: {fromNodeId} -> {toNodeId}, L: {load.Value:F3}, S: {stability.Value:F3}, E: {efficiency.Value:F3}");
            }
            else
            {
                Debug.Log($"[Telemetry] Platform: {platform}, Segment: {fromNodeId} -> {toNodeId}, Traversable: False (L/S/E: null)");
            }
        }
